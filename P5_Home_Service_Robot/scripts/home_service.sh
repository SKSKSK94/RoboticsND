#!/bin/sh
export TURTLEBOT3_MODEL=waffle
export TURTLEBOT3_GAZEBO_WORLD_FILE=$(pwd)/src/RoboticsND/P5_Home_Service_Robot/worlds/home_service.world
map_file=$(pwd)/src/RoboticsND/P5_Home_Service_Robot/maps/home_service.yaml

xterm  -e  " source devel/setup.bash; roslaunch spawn_home_service_world turtlebot3_home_service_world.launch " &
sleep 3
xterm  -e  " source devel/setup.bash; roslaunch turtlebot3_navigation turtlebot3_navigation.launch map_file:=${map_file} open_rviz:=false" &
sleep 3
xterm  -e  " source devel/setup.bash; roslaunch add_markers add_markers.launch " &
sleep 3
xterm  -e  " source devel/setup.bash; roslaunch pick_objects pick_objects.launch "